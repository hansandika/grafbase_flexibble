...
const Project = async ({ params: { id } }: { params: { id: string } }) => {
  ...
  if (!result.pg.projects) {
    return (
      <p>Failed to Fetch Project</p>
    )
  }

  const projectDetails = result?.pg.projects

  const renderLink = () => `/profile/${projectDetails?.users?.userId}`
  ...
  return (
    <Modal>
      <section className="flexBetween gap-y-8 max-w-4xl max-xs:flex-col w-full">
        <div className="flex-1 flex items-start gap-5 w-full max-xs:flex-col">
          <Link href={renderLink()}>
            <Image
              src={projectDetails?.users?.avatarUrl}
              width={50}
              height={50}
              alt="profile"
        ...
        {session?.user?.email === projectDetails?.users?.email && (
          <div className="flex justify-end items-center gap-2">
            <ProjectActions projectId={projectDetails?.projectId} />
          </div>
        )}
      </section>
      ...
      <RelatedProjects userId={projectDetails?.users?.userId} projectId={projectDetails?.projectId} />
    </Modal>
  )
}
